...
import ua.kpi.personal.repo.CategoryClient; 
...
public class CategoriesController {
    ...
    private void refresh(){
        if (user != null) {
            ...
            List<Category> allCategories = categoryClient.findByUserId(user.getId()); 
    ...
    private void onAdd(){
        ...
        if (editingCategory != null) {
            ...
            if (categoryClient.update(updatedCategory)) { 
            ...
            Category created = categoryClient.create(newCategory); 
            
            if(created!=null){
                messageLabel.setText("Додана категорія: " + created.getName());
                refresh();
            } else {
                messageLabel.setText("Помилка збереження в API."); // Змінюємо текст помилки
            }
        }
    }
    ...
    private void onDelete() {
        ...
        if (categoryClient.delete(selectedCategory.getId())) { 
            messageLabel.setText("Категорія '" + selectedCategory.getName() + "' видалена.");
            refresh();
        } else {
            messageLabel.setText("Помилка видалення категорії. Можливо, є пов'язані транзакції.");
        }
    }
